  ...
  try {
    ...
    if (mode === "signin") {
      ...
      if (mode === "signin") {
        ...
        const result = await signUp({ email: values.email, password: values.password }, values.fullName || "");
    ...
    <main className="min-h-[100dvh] bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <div className="flex items-center gap-2 mb-8 justify-center">
          ...
          <span className="text-2xl font-bold tracking-tight">FitPilot<span className="text-primary">AI</span></span>
        </div>
        <Card className="border-border shadow-xl">
          <CardHeader>
            <CardTitle>{mode === "signin" ? "Sign in to your gym" : "Create your gym account"}</CardTitle>
            <CardDescription>
              {mode === "signin" ? "Use your Supabase account to continue." : "Your gym workspace starts with your account."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(submit)} className="space-y-4">
                {mode === "signup" && (
                  <FormField control={form.control} name="fullName" render={({ field }) => (
                    <FormItem><FormLabel>Full name</FormLabel><FormControl><div className="relative"><UserRound className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" placeholder="Alex Johnson" {...field} /></div></FormControl><FormMessage /></FormItem>
                  )} />
                )}
                <FormField control={form.control} name="email" render={({ field }) => (
                  <FormItem><FormLabel>Email</FormLabel><FormControl><div className="relative"><Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" type="email" placeholder="you@gym.com" {...field} /></div></FormControl><FormMessage /></FormItem>
                )} />
...
>
  Test Button
            ...
            <button type="button" className="w-full text-sm text-muted-foreground hover:text-foreground mt-5" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); form.reset(); }}>
              {mode === "signin" ? "New to FitPilot? Create an account" : "Already have an account? Sign in"}
            </button>
          </CardContent>
        </Card>
      </div>
